...
class UserMarketplaceAdapter(
    posts: ArrayList<SalePost>,
    callback: MarketplacePostClickInterface
) : RecyclerView.Adapter<UserMarketplaceAdapter.MarketplaceViewHolder>() {

    private var posts = emptyList<SalePost>()
    private var mCallback: MarketplacePostClickInterface? = null
   ...
   /* fun setData(data: ArrayList<MarketPlacePost>) {
    ...
    inner class MarketplaceViewHolder(private val binding: AdapterUserMarketplaceBinding) :
        ...
        fun bindView(post: SalePost) {
            ...
            Glide.with(mContext)
                .load(Global.getImageUrl(post.productCoverImg))
    ...
    interface MarketplacePostClickInterface {
        fun onPostClick(post: SalePost)
    }
